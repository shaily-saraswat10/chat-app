        ...
        <form  onSubmit={(onSubmitHandler)} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
            ...
            {currState === 'Sign up' && isDataSubmitted && (
                 <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
                  rows={4} className='p-2 border border-gray-500 rounded-md focus:outlline-none focus:ring-2 focus:ring-indigo-500' placeholder='Provide a short bio...' required ></textarea>
              )
            }

            <button className='py-3 bg-gradient-to-r from-purple-400 to-voilet-600 text-white rounded-md cursor-pointer'>
